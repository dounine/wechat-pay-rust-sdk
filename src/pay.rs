...
pub trait PayNotifyTrait: WechatPayTrait {
    fn verify_signatrue<S>(&self, pub_key: &str, timestamp: S, nonce: S, signatrue: S, body: S) -> Result<(), PayError>
        where S: AsRef<str>
    {
        let message = format!("{}\n{}\n{}\n", timestamp.as_ref(), nonce.as_ref(), body.as_ref());
        ...
        pub_key.verify(scheme, &hashed, signatrue.as_slice())
...
mod tests {
    ...
    fn test_pay_verify_sign() {
        ...
        let hashed = rsa::sha2::Sha256::new()
            ...
            .finalize();
        let scheme = Pkcs1v15Sign::new::<Sha256>();
        pub_key.verify(scheme, &hashed, signature.as_slice()).expect("签名验证失败");
